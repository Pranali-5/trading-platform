...
const POPULAR_SYMBOLS = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA'];
    ...
    <div className="card p-4 animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className={`p-1.5 rounded-lg transition-all ${isCreating ? 'bg-loss/10 text-loss' : 'bg-secondary hover:bg-secondary-hover text-foreground-muted'}`}
        >
          {isCreating ? (
            ...
            <div className="space-y-2 animate-scale-in">
              <div className="relative">
                <input
                  ...
                  autoFocus
                />
                <svg className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-foreground-subtle" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  {searchResults.map((result) => (
                    ...
                    >
                      <div>
                        ...
                        <span className="text-xs text-foreground-muted ml-2">{result.name}</span>
                      </div>
              ...
              <div className="flex gap-1 flex-wrap">
                {POPULAR_SYMBOLS.filter(s => !watchlistItems.some(i => i.symbol === s)).slice(0, 5).map(symbol => (
                  <button
                    key={symbol}
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto">
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-xs text-foreground-subtle">Add stocks to track</p>
            </div>
          ) : (
            watchlistItems.map((item, idx) => {
              const ticker = latestTickers[item.symbol];
              const isPositive = ticker && ticker.price > (ticker.open || ticker.price);
