...
export default function Dashboard({ selectedSymbol: propSelectedSymbol }: DashboardProps) {
  const tickers = useTickers(50);
  const [selectedSymbol, setSelectedSymbol] = useState<string | null>(propSelectedSymbol || null);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Gainers"
          value={uniqueTickers.filter(t => (t.changePercent || 0) > 0).length.toString()}
          trend="up"
          icon={
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {uniqueTickers.length === 0 ? (
              <div className="text-center py-12">
                <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-secondary flex items-center justify-center">
                  <svg className="w-8 h-8 text-foreground-subtle animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                </div>
                <p className="text-sm text-foreground-muted">Connecting to market...</p>
                <p className="text-xs text-foreground-subtle mt-1">Waiting for live data</p>
              </div>
            ) : (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
              ...
              {selectedTicker ? (
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center">
                    <span className="text-sm font-bold text-primary">{selectedTicker.symbol.slice(0, 2)}</span>
                  </div>
                  <div>
                    <h2 className="text-lg font-bold text-foreground">{selectedTicker.symbol}</h2>
                ...
                <h2 className="text-lg font-semibold text-foreground">Select a stock</h2>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex bg-secondary rounded-lg p-1">
                ...
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19V5M5 8h8M5 16h8M15 19V5m4 3h-8m8 8h-8" />
                  </svg>
                </button>
                <button
                ...
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7v10M16 7v10M8 12h8" />
                  </svg>
                </button>
              </div>
            </div>
          ...
          {selectedSymbol && chartData.length > 0 ? (
              ...
              {selectedTicker && selectedTicker.open && (
                <div className="absolute top-2 right-2 bg-surface/90 backdrop-blur-sm rounded-lg p-2 text-xs border border-border">
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                    ...
                    <span className="font-mono tabular-nums">${selectedTicker.open?.toFixed(2)}</span>
                    <span className="text-foreground-muted">High</span>
                    <span className="font-mono tabular-nums text-profit">${selectedTicker.high?.toFixed(2)}</span>
                    <span className="text-foreground-muted">Low</span>
                    <span className="font-mono tabular-nums text-loss">${selectedTicker.low?.toFixed(2)}</span>
                    <span className="text-foreground-muted">Vol</span>
                    <span className="font-mono tabular-nums">{selectedTicker.volume?.toLocaleString()}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-center h-80 bg-secondary/50 rounded-xl border-2 border-dashed border-border">
              <div className="text-center">
                <svg className="w-16 h-16 mx-auto text-foreground-subtle opacity-50 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <p className="text-foreground-muted font-medium">
                  {selectedSymbol ? 'Loading chart data...' : 'Select a stock to view chart'}
                </p>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {ticker.volume && (
              <p className="text-xs text-foreground-subtle">Vol: {(ticker.volume / 1000000).toFixed(1)}M</p>
            )}
          </div>
        </div>
        <div className="text-right">
          ...
          <p className={`text-xs font-medium ${isPositive ? 'text-profit' : 'text-loss'}`}>
            {isPositive ? '▲' : '▼'} {Math.abs(ticker.changePercent || 0).toFixed(2)}%
          </p>
        </div>
      </div>
    </div>
